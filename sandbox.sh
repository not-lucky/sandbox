...
show_help() {
    echo "================================================================="
    echo "    Identity-Based Secure Workspace Manager"
    echo "================================================================="
    echo "Usage: $0 [options] -- <command> [arguments...]"
    echo ""
    echo "Options:"
    echo "  -i, --identity <name>    Unique identity name (Creates persistent hardware spoof & home)"
    echo "  -w, --whitelist <paths>  Comma-separated list of host files/dirs to expose"
    echo "  -p, --port <number>      Host SOCKS5 proxy port (Default: 10808)"
    echo "  -f, --profile <name>     Explicitly use/override pre-existing Firejail profile"
    echo "  --gui                    Enable GUI support (X11 & Wayland forwarding)"
    echo "  -h, --help               Show this help menu"
    echo ""
    echo "Examples:"
    echo "  # Launch Trae as 'Account 1' (Persistent login, spoofed hardware):"
    echo "  $0 -i trae-acc1 --gui -w ~/.agents -- /usr/share/trae/trae --no-sandbox"
    echo ""
    echo "  # Run a CLI agent with isolated persistent state:"
    echo "  $0 -i qoder_astral -w ~/.agents -- qodercli"
    echo "================================================================="
    exit 0
}
...
SESSION_HASH=${HASH:0:6}

# Generate a valid, persistent MAC Address
MAC_ADDR="02:${HASH:2:2}:${HASH:4:2}:${HASH:6:2}:${HASH:8:2}:${HASH:10:2}"

# Network Params
NS_NAME="ns-$SESSION_HASH"
VETH_HOST="vh-$SESSION_HASH"
VETH_NS="vn-$SESSION_HASH"
OCTET=$(( 1 + 0x${HASH:0:2} % 254 ))
...
for cmd in firejail socat tun2socks ip python3; do
    if ! command -v $cmd &> /dev/null; then
        echo "Error: System dependency '$cmd' is missing."
        exit 1
    fi
done

cleanup() {
    echo -e "\nSaving state and cleaning up identity [$IDENTITY]..."
    ...
    rm -f "$PROFILE_PATH" "$DNS_PROXY_PY"
    echo "Cleanup complete."
}
trap cleanup EXIT ERR INT TERM

# --- 5. NETWORK NAMESPACE SETTINGS ---
echo "Initializing hardware-spoofed network for identity [$IDENTITY]..."
...
CMD_PATH=$(which "$CMD_NAME" 2>/dev/null || realpath "$CMD_NAME" 2>/dev/null)
...
echo "Spoofed MAC:     $MAC_ADDR"
echo "State Directory: $HOME_DIR"
echo "--------------------------------------------------------"

...
| `internal/sandbox/executor.go` | `ExecuteOptions` shape, `applyElectronWorkarounds`, `buildEnvArgs`, `buildCommandArgs`, `Execute`. **Run `gofmt -w` on this — it was flagged by audit.** |
| `internal/sandbox/firejail.go` | `BuildProfileContent(identity, nativeProfile, cmdName)` generates the runtime profile (hostname, machine-id, `/sys/class/dmi` blocklist, private-etc, read-only bins, X11 whitelist). For Electron apps, emits a relaxed seccomp profile allowing bwrap syscalls. |
